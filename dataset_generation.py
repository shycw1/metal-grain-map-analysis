...
for idx in range(dataset_volume):
    ...
    fig.savefig(f"dataset/original/{idx+1}.png", dpi=map_pixel_size/10, bbox_inches='tight', pad_inches=0, format='png', transparent=True, facecolor='none')
    ...
    image = plt.imread(f'dataset/original/{idx+1}.png')[:, :, :3]  # keep only the first three channels
    ...
    plt.imsave(f"dataset/noisy/{idx+1}.png", noisy_image)

    print(f"[Info] image {idx+1} with {n} seeds, SNR: {snr} dB, Grain Number: {number_of_centroids}, mean grain size : {mean_grain_size} mm^2 Generated")
